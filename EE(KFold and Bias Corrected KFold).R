...
# Read the Excel file
data <- read_excel("d:\\Sami\\Documents\\Statistics\\ENB2012_data.xlsx")
...
svm_model <- train(`Y2` ~ ., data = data, method = "svmRadial", trControl = train_control)
...
# ************* Fold 5  ****************************#
# Set up the training control
train_control <- trainControl(method = "cv", number = 5)  # Change 'number' for different fold numbers
# Train SVM model with cross-validation
svm_model <- train(`Y2` ~ ., data = data, method = "svmRadial", trControl = train_control)
...
# *********************  Determine the number of observations per fold 3 **************#
...
for (fold in 1:3) {
  ...
  svm_model <- svm(`Y2` ~ ., data = train_data, kernel = "radial")
  
  # Make predictions on test data
  predictions <- predict(svm_model, test_data)
  
  # Calculate mean squared error
  mse_values[fold] <- mse_func(predictions, test_data$`Y2`)
